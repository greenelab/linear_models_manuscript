...
We first simulated three datasets to better understand model performance for a variety of linear or non-linear data generating processes.
...
Similarly, for multiclass problems logistic regression performing poorly, while the non-linear models had performance that increased with an increase in data while remaining worse than before the linear signal was removed (fig. @fig:signal_removed A,C).
